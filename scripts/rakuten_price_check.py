...
def parse_unit(item, mode, count_hint):
    ...
    for field in ("catchcopy", "itemName"):
        ...
        if not s:
            continue
        # 「24本/48本」のような選択式は価格が特定できないので使わない
        if re.search(r"\d{1,3}\s*本\s*[/・、]\s*\d{1,3}\s*本", s):
            continue
        if mode == "weight":
            m = re.search(r"(?:約)?\s*(\d+(?:\.\d+)?)\s*(?:kg|Kg|KG|キロ)", s)
            if m:
                kg = float(m.group(1))
                if 0.3 <= kg <= 30:
                    return price / kg, f"{kg}kg"
            m = re.search(r"(?:約)?\s*(\d{3,5})\s*g\b", s)
            if m:
...
def print_report(item, res):
    ...
    print(f"   価格帯 {band} ／ 楽天の該当 {res['total']:,}件"
          f" ／ 表示 {len(res['rows'])}件（明らかに別商材 {res['noise']}件は除外）")
    print()
    ...
    print(f"   {'価格':>9}  {'参考単価':>12}  商品 / ショップ")
    print("   " + "-" * 71)
    for r in res["rows"][:15]:
        u = f"{r['unit']:,.0f}{unit_name}（{r['unit_label']}）" if r["unit"] else "―"
        print(f"   ¥{r['price']:>8,}  {u:>12}  {r['name'][:40]}")
        rv = f"  ★{r['review_avg']}({r['review_count']})" if r.get("review_count") else ""
        print(f"   {'':>9}  {'':>12}  └ {r['shop'][:26]}{rv}")
    print()
